import re
import argparse
import matplotlib.pyplot as plt
import numpy as np


def running_mean(x, n):
    cumsum = np.cumsum(np.insert(x, 0, 0))
    return (cumsum[n:] - cumsum[:-n]) / float(n)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('log_files', type=str, nargs='+')
    parser.add_argument('--plot_sr', default=True, action='store_true')
    parser.add_argument('--plot_cr', default=True, action='store_true')
    parser.add_argument('--plot_time', default=True, action='store_true')
    parser.add_argument('--plot_reward', default=True, action='store_true')
    parser.add_argument('--plot_pl', default=True, action='store_true')
    parser.add_argument('--plot_itr', default=True, action='store_true')
    parser.add_argument('--plot_sd', default=True, action='store_true')
    parser.add_argument('--plot_train', default=False, action='store_true')
    parser.add_argument('--plot_val', default=False, action='store_true')
    parser.add_argument('--plot_test', default=True, action='store_true')
    parser.add_argument('--window_size', type=int, default=200)
    args = parser.parse_args()

    # define the names of the models you want to plot and the longest episodes you want to show
    models = ['1', '2', '3', '4']
    max_episodes = 10000

    ax1 = ax2 = ax3 = ax4 = ax5 = ax6 = ax7 = None
    ax1_legends = []
    ax2_legends = []
    ax3_legends = []
    ax4_legends = []
    ax5_legends = []
    ax6_legends = []
    ax7_legends = []

    log_file_lidar = args.log_files[0]
    log_file_ground = args.log_files[0]

    # for i, log_file in enumerate(args.log_files):
    with open(log_file_lidar, 'r') as file:
        log_lidar = file.read()
    with open(log_file_ground, 'r') as file:
        log_ground = file.read()
    
    # VALIDATION
    # val_pattern = r"VAL   in episode (?P<episode>\d+) has success rate: (?P<sr>[0-1].\d+), " \
    #               r"collision rate: (?P<cr>[0-1].\d+), nav time: (?P<time>\d+.\d+), " \
    #               r"total reward: (?P<reward>[-+]?\d+.\d+)"
    # val_episode = []
    # val_sr = []
    # val_cr = []
    # val_time = []
    # val_reward = []
    # for r in re.findall(val_pattern, log):
    #     val_episode.append(int(r[0]))
    #     val_sr.append(float(r[1]))
    #     val_cr.append(float(r[2]))
    #     val_time.append(float(r[3]))
    #     val_reward.append(float(r[4]))

    # TESTING
    test_pattern = r"TEST  in episode (?P<episode>\d+) has success rate: (?P<sr>[0-1].\d+), " \
                    r"collision rate: (?P<cr>[0-1].\d+), nav time: (?P<time>\d+.\d+), " \
                    r"total reward: (?P<reward>[-+]?\d+.\d+), "  \
                    r"path length: (?P<pl>[-+]?\d+.\d+), " \
                    r"intrusion time ratio \(ITR\): (?P<itr>[-+]?\d+.\d+), " \
                    r"and social distance during intrusions \(SD\): (?P<sd>[-+]?\d+.\d+)"


    # LIDAR
    test_episode = []
    test_sr = []
    test_cr = []
    test_time = []
    test_reward = []
    test_pl = []
    test_itr = []
    test_sd = []
    for r in re.findall(test_pattern, log_lidar):
        test_episode.append(int(r[0]))
        test_sr.append(float(r[1]))
        test_cr.append(float(r[2]))
        test_time.append(float(r[3]))
        test_reward.append(float(r[4]))

        test_pl.append(float(r[5]))
        test_itr.append(float(r[6]))
        test_sd.append(float(r[7]))

    # GROUND TRUTH
    test_episode_ground = []
    test_sr_ground = []
    test_cr_ground = []
    test_time_ground = []
    test_reward_ground = []
    test_pl_ground = []
    test_itr_ground = []
    test_sd_ground = []
    for r in re.findall(test_pattern, log_ground):
        test_episode_ground.append(int(r[0]))
        test_sr_ground.append(float(r[1]))
        test_cr_ground.append(float(r[2]))
        test_time_ground.append(float(r[3]))
        test_reward_ground.append(float(r[4]))

        test_pl_ground.append(float(r[5]))
        test_itr_ground.append(float(r[6]))
        test_sd_ground.append(float(r[7]))

    # print("success rate:",test_sr)
    # print("collision rate:",test_cr)
    # print("nav time:",test_time)
    # print("reward:",test_reward)
    # print("eps:",test_episode)

    # TRAINING
    # train_pattern = r"TRAIN in episode (?P<episode>\d+) has success rate: (?P<sr>[0-1].\d+), " \
    #                 r"collision rate: (?P<cr>[0-1].\d+), nav time: (?P<time>\d+.\d+), " \
    #                 r"total reward: (?P<reward>[-+]?\d+.\d+)"
    # train_episode = []
    # train_sr = []
    # train_cr = []
    # train_time = []
    # train_reward = []
    # for r in re.findall(train_pattern, log):
    #     train_episode.append(int(r[0]))
    #     train_sr.append(float(r[1]))
    #     train_cr.append(float(r[2]))
    #     train_time.append(float(r[3]))
    #     train_reward.append(float(r[4]))
    # train_episode = train_episode[:max_episodes]
    # train_sr = train_sr[:max_episodes]
    # train_cr = train_cr[:max_episodes]
    # train_time = train_time[:max_episodes]
    # train_reward = train_reward[:max_episodes]

    # # smooth training plot
    # train_sr_smooth = running_mean(train_sr, args.window_size)
    # train_cr_smooth = running_mean(train_cr, args.window_size)
    # train_time_smooth = running_mean(train_time, args.window_size)
    # train_reward_smooth = running_mean(train_reward, args.window_size)


    # average navigation time
    # avg_nav_time_train = sum(train_time) / len(train_time)
    # avg_nav_time_val = sum(val_time) / len(val_time)
    avg_nav_time_test = sum(test_time) / len(test_time)

    # print("Average navigation time (training):",avg_nav_time_train)
    # print("Average navigation time (validation):",avg_nav_time_val)
    print("Average navigation time (test):",avg_nav_time_test)

    
    # plot sr
    if args.plot_sr:
        if ax1 is None:
            _, ax1 = plt.subplots()
        if args.plot_train:
            ax1.plot(range(len(train_sr_smooth)), train_sr_smooth)
            # ax1_legends.append(models[i])
            ax1_legends.append("Training")
        if args.plot_val:
            ax1.plot(val_episode, val_sr)
            # ax1_legends.append(models[i])
            ax1_legends.append("Validation")
        if args.plot_test:
            ax1.plot(test_episode, test_sr, label="LiDAR")
            ax1.plot(test_episode_ground, test_sr_ground, label="Ground truth")
            
            # ax1_legends.append(models[i])
            # ax1_legends.append("Testing")

        ax1.legend()
        ax1.set_xlabel('Episodes')
        ax1.set_ylabel('Success Rate')
        ax1.set_title('Success Rate (SR)')

    # plot navigation time
    if args.plot_time:
        if ax2 is None:
            _, ax2 = plt.subplots()
        if args.plot_train:
            ax2.plot(range(len(train_time_smooth)), train_time_smooth)
            # ax2_legends.append(models[i])
            ax2_legends.append("Training")
        if args.plot_val:
            ax2.plot(val_episode, val_time)
            # ax2_legends.append(models[i])
            ax2_legends.append("Validation")
        if args.plot_test:
            ax2.plot(test_episode, test_time, label="LiDAR")
            ax2.plot(test_episode_ground, test_time_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax2_legends.append("Testing")

        ax2.legend()
        ax2.set_xlabel('Episodes')
        ax2.set_ylabel('Time(s)')
        ax2.set_title("Robot Navigation Time (successful episodes)")

    # plot cr
    if args.plot_cr:
        if ax3 is None:
            _, ax3 = plt.subplots()
        if args.plot_train:
            ax3.plot(range(len(train_cr_smooth)), train_cr_smooth)
            # ax3_legends.append(models[i])
            ax3_legends.append("Training")
        if args.plot_val:
            ax3.plot(val_episode, val_cr)
            # ax3_legends.append(models[i])
            ax3_legends.append("Validation")
        if args.plot_test:
            ax3.plot(test_episode, test_cr, label="LiDAR")
            ax3.plot(test_episode_ground, test_cr_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax3_legends.append("Testing")

        ax3.legend()
        ax3.set_xlabel('Episodes')
        ax3.set_ylabel('Collision Rate')
        ax3.set_title('Collision Rate')

    # plot reward
    if args.plot_reward:
        if ax4 is None:
            _, ax4 = plt.subplots()
        if args.plot_train:
            ax4.plot(range(len(train_reward_smooth)), train_reward_smooth)
            # ax4_legends.append(models[i])
            ax4_legends.append("Training")
        if args.plot_val:
            ax4.plot(val_episode, val_reward)
            # ax4_legends.append(models[i])
            ax4_legends.append("Validation")
        if args.plot_test:
            ax4.plot(test_episode, test_reward, label="LiDAR")
            ax4.plot(test_episode_ground, test_reward_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax4_legends.append("Testing")

        ax4.legend()
        ax4.set_xlabel('Episodes')
        ax4.set_ylabel('Reward')
        ax4.set_title('Cumulative Discounted Reward')

    # plot path length
    if args.plot_pl:
        if ax5 is None:
            _, ax5 = plt.subplots()
        # if args.plot_train:
        #     ax4.plot(range(len(train_reward_smooth)), train_reward_smooth)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Training")
        # if args.plot_val:
        #     ax4.plot(val_episode, val_reward)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Validation")
        if args.plot_test:
            ax5.plot(test_episode, test_pl, label="LiDAR")
            ax5.plot(test_episode_ground, test_pl_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax5_legends.append("Testing")

        ax5.legend()
        ax5.set_xlabel('Episodes')
        ax5.set_ylabel('Reward')
        ax5.set_title('Robot Path Length (PL)')

    # plot ITR
    if args.plot_itr:
        if ax6 is None:
            _, ax6 = plt.subplots()
        # if args.plot_train:
        #     ax4.plot(range(len(train_reward_smooth)), train_reward_smooth)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Training")
        # if args.plot_val:
        #     ax4.plot(val_episode, val_reward)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Validation")
        if args.plot_test:
            ax6.plot(test_episode, test_itr, label="LiDAR")
            ax6.plot(test_episode_ground, test_itr_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax6_legends.append("Testing")

        ax6.legend()
        ax6.set_xlabel('Episodes')
        ax6.set_ylabel('Reward')
        ax6.set_title('Intrusion Time Ratio (ITR)')

    # plot SD
    if args.plot_sd:
        if ax7 is None:
            _, ax7 = plt.subplots()
        # if args.plot_train:
        #     ax4.plot(range(len(train_reward_smooth)), train_reward_smooth)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Training")
        # if args.plot_val:
        #     ax4.plot(val_episode, val_reward)
        #     # ax4_legends.append(models[i])
        #     ax4_legends.append("Validation")
        if args.plot_test:
            ax7.plot(test_episode, test_sd, label="LiDAR")
            ax7.plot(test_episode_ground, test_sd_ground, label="Ground truth")
            # ax1_legends.append(models[i])
            # ax7_legends.append("Testing")

        ax7.legend()
        ax7.set_xlabel('Episodes')
        ax7.set_ylabel('Reward')
        ax7.set_title('Social Distance during Intrusions (SD)')

    plt.show()



if __name__ == '__main__':
    main()
