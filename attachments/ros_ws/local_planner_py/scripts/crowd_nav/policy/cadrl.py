...
def mlp2(input_dim, mlp_dims, last_relu=False):
    layers = []
    mlp_dims = [input_dim] + mlp_dims
    for i in range(len(mlp_dims) - 1):
        layers.append(nn.Linear(mlp_dims[i], mlp_dims[i + 1]))
...
def bimlp(input_dim, mlp_dims, last_relu=False):
    layers = []
    mlp_dims = [input_dim] + mlp_dims
    layers.append(nn.Linear(mlp_dims[0], mlp_dims[1]))
    for i in range(len(mlp_dims) - 2):
        layers.append(nn.Linear(mlp_dims[i+1], mlp_dims[i + 2]))
        if i != len(mlp_dims) - 2 or last_relu:
            layers.append(nn.ReLU())
    net = nn.Sequential(*layers)
    return net
...
class CADRL(Policy):
    ...
    def build_action_space(self, v_pref):
        ...
        speeds = [(np.exp((i + 1.0) / self.speed_samples) - 1.0) / (np.e - 1.0) * v_pref for i in range(self.speed_samples)]
        # speeds = [(np.exp((i + 1.0) / self.speed_samples) - 1.0) / (np.e - 1.0) * v_pref for i in range(3)]
        
        # # Use backward velocities
        #For carla
        # speeds = [0.6, 1.0]
        #for V6
        # speeds_reverse = list(-1.0 * np.array(speeds[:-1]))
        # speeds_reverse.reverse()
        # speeds = speeds + speeds_reverse
        ###
        ...
        if holonomic:
            ...
            # rotations = [(np.exp((i + 1) / self.rotation_samples) - 1) / (np.e - 1) *  np.pi / 6 for i in range(self.rotation_samples)]
            # rotations = [0] + rotations + list(-1.0 * np.array(rotations))
    
        #for V6
        # if holonomic:
        #     rotations_db = np.linspace(0, 2 * np.pi, self.rotation_samples, endpoint=False)
        # else:
        #     rotations_db = np.linspace(-np.pi / 8, np.pi / 8, self.rotation_samples)
        # rotations = rotations_db[0] + rotations_db[::3] + rotations_db[-1]
        # rotations = np.concatenate([rotations, np.array([0])], axis =0)
        ###
