...
def conv_mlp(input_dim, mlp_dims, last_relu=False):
    layers = []
    mlp_dims = [input_dim] + mlp_dims
    for i in range(len(mlp_dims) - 1):
        layers.append(nn.Conv1d(mlp_dims[i], mlp_dims[i + 1], kernel_size=1))
...
def conv_mlp2(input_dim, mlp_dims, kernel_size=(1, 1), last_relu=False):
    layers = []
    mlp_dims = [input_dim] + mlp_dims
    for i in range(len(mlp_dims) - 1):
        layers.append(nn.Conv2d(mlp_dims[i], mlp_dims[i + 1], kernel_size=(1, 1)))
        if i != len(mlp_dims) - 2 or last_relu:
            # layers.append(nn.BatchNorm1d(mlp_dims[i + 1]))
            layers.append(nn.ReLU())
    net = nn.Sequential(*layers)
...
class CADRL(Policy):
    ...
    def build_action_space(self, v_pref):
        ...
        if holonomic:
            ...
            rotations = np.linspace(-np.pi / 6, np.pi / 6, self.rotation_samples)
    ...
    def propagate(self, state, action, scale=1.0):
        if isinstance(state, ObservableState):
            # propagate state of humans
            next_px = state.px + action.vx * self.time_step * scale
            next_py = state.py + action.vy * self.time_step * scale
            ...
            if self.kinematics == 'holonomic':
                ...
                next_px = state.px + next_vx * self.time_step * scale
                next_py = state.py + next_vy * self.time_step * scale
