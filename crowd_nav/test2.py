...
def main():
    ...
    robot.print_info()
    if args.visualize:
        ob, lidar_image = env.reset(args.phase, args.test_case)
        done = False
        last_pos = np.array(robot.get_position())
        while not done:
            action = robot.act(ob, lidar_image)
            ob, lidar_image, _, done, info = env.step(action)
